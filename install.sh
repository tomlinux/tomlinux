...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate -t 3 -T 10 https://install.direct/go.sh
...
domain_check() {
    ...
    local_ip=$(curl -s  http://www.net.cn/static/customercare/yourip.asp  | grep -Po "<h2>.*</h2>" | sed  's#<h2>##g' | sed  's#</h2>##g')
...
acme() {
    ...
    # sed  -i 's#http://$d/.well-known#http://$d:8089/.well-known#g' "$HOME"/.acme.sh/acme.sh
...
acme_dns() {
     ...
     if [ ! -f "$HOME"/.acme.sh/${domain)/fullchain.cer } ]
     then  
        "$HOME"/.acme.sh/acme.sh  --issue  --dns  -d "${domain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please
     fi
     read -rp "please 解析${domain} 到如上记录。解析完成，请按Y 继续执行后面操作或者goto 跳过！！(Y/N) " dns_option
     case $dns_option in
        [yY][eE][sS] | [yY])
            "$HOME"/.acme.sh/acme.sh  --renew   -d "${domain}" -k ec-256 --force --yes-I-know-dns-manual-mode-enough-go-ahead-please
            ret=`echo $?`
            ;;
        goto)
            echo "已经跳过执行域名${domain}的生成"
        ;;
        *) 
            echo "等待解析，5分钟执行之后执行后面操作"
            exit 1 
            ;;
        esac
        ...
        echo -e "${OK} ${GreenBG} SSL 证书生成成功 ${Font}"
        mkdir /data
        if "$HOME"/.acme.sh/acme.sh --installcert -d "${domain}" --fullchainpath /data/v2ray.crt --keypath /data/v2ray.key  --force; then
            echo -e "${OK} ${GreenBG} 证书配置成功 ${Font}"
        else
            echo -e "${Error} ${RedBG} SSL 证书生成失败 ${Font}"
...
bbr_boost_sh() {
    ...
    wget -N --no-check-certificate -t 3 -T 10 "https://github.com/ylx2016/Linux-NetSpeed/releases/download/sh/tcp.sh" && chmod +x tcp.sh && ./tcp.sh
}
mtproxy_sh() {
    [ -f "mtproxy_go.sh" ] && rm -rf ./mtproxy_go.sh
    wget -N --no-check-certificate -t 3 -T 10 https://github.com/whunt1/onekeymakemtg/raw/master/mtproxy_go.sh && chmod +x mtproxy_go.sh && ./mtproxy_go.sh
}
...
update_sh() {
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate -t 3 -T 10 https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh
...
menu() {
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/tomlinux/tomlinux/master/go.sh)
        ;;
    4)
